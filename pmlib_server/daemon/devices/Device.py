...
class Device(threading.Thread):
    ...
    def add_line(self, number, name, voltage, description):
        if self.lines.has_key(number):
            ...
            raise SyntaxError, msg
        self.lines[number]= Line(number, name, voltage, description)
...
class Line(object):
    ...
    # @param [in] description A text description of the line
    #
    def __init__(self, number, name, voltage, description):
        self.number= number
        self.name= name
    ...
    def __repr__(self):
        return "Line {0} (name: {1}, voltage: {2}, description: '{3}')".format(self.number, self.name, self.voltage, self.description)
...
## A INA line description
#
class INALine(Line):

    ## Creates a INA line description
    #
    # @param [in] name        The line name (used for identification)
    # @param [in] computer    The computer this INA line is attached to
    # @param [in] voltage     The line voltage
    # @param [in] description An optional text description of the line
    #
    def __init__(self, number, name, url, computer, voltage, description=""):
        if not isinstance(computer, daemon.modules.Computer) and computer != None:
            msg="the given computer parameter is not a Computer object"
            raise SyntaxError, msg
        super(INALine, self).__init__(number, name, voltage, description)
	self.url = url
        if computer:
            self.computer= computer
        else:
