...
/*	pm_set_server("150.128.83.55", 6526, &server);
	pm_set_lines("5-8", &lines);
	pm_create_counter("DCMeter1", lines, !aggregate, frequency, server, &contador);
*/
	pm_set_server("150.128.80.126", 6526, &server);
	LINE_SET_ALL(&lines);
	pm_create_counter("NI9205", lines, aggregate, frequency, server, &contador);
