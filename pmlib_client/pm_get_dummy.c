...
int pm_get_dummy(char *file_name, counter_t *pm_counter, line_t lines, int set, unit_t unit){
...
/* Store watts_sets_size */
  ...
  watts_len = (pm_counter->measures)->energy.watts_size*sizeof(float);	
 ...
 /* Receive the rest of message */
  ...
  {	//Only aggregate value will be printed
	...
	fseek(file_data, -1, SEEK_END);
	elem_watts =1234.1234;
	fprintf(file_data, "\n");
	fflush(file_data);
	fsync(fileno(file_data));
	
	(pm_counter->measures)->energy.watts_size=75000000;
	ini = 0;
	fin = 75000000;
	read=0;
